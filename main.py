...
def reload_or_start_new() -> tuple[int, Player.Player, list[
    list[Tile.Tile]]]:  # leaving as skeleton, might want to incorporate into quit input (or its own)
    ...
    if answer in ("n", "new game", "new"):
        ...
        try:
            ...
            with open(fileName + ".json", "r") as file:
                ...
                player.minerals_in_bag = [Minerals.mineralTypes[mineral] for mineral in
                                          reader["player"]["minerals_in_bag"]]
...
def drop_item(tilePos: tuple[int, int]):
    ...
    new_item = map[tilePos[1]][tilePos[0]].item
    dropped_item: Constants.Items = player.grab_item(new_item)

    # If grab_item returns None, inventory is full and we need to ask the user which item to drop
...
def maulwurf_remains(monster: Minerals.mineralTypes):
    ...
    print(
        f"{ansi.b_italics("Gathered Resource")}: {ansi.g_bold(monster.description)} ({ansi.g_bold(f"+{monster.score_value:,}!")}) --> {ansi.cyan(f"Current Score: {player.total_score:,}")}")


def use_mushroom(map: list[list[Tile.Tile]], pos: tuple[int, int]) -> None:  # skeleton based off of mini map
    print(ansi.italics(
        "As the psilocybe slips down your gullet, you feel a heightened connection to the caverns around you"),
    ...
    if pos[0] in (
    range(3)):  # defines which item will be shown on the mini map based off player.pos (similar way to row)
        item_bounds = (0, 5)
...
def inspect_tile(tilePos: tuple[int, int]):
    ...
    if map[tilePos[1]][tilePos[0]].item != map[tilePos[1]][
        tilePos[0]].item.nothing:  # if there is an item on the player's tile
...
def check_pos(player: Player.Player):  # didn't want player.pos in Tile.py
    global running
    if map[player.pos[1]][player.pos[0]].hasMaulwurf or map[player.pos[1]][
        player.pos[0]].cavedIn:  # checks if the tile is valid
        ...
        elif map[player.pos[1]][
            player.pos[0]].hasMaulwurf:  # if it is not caved in, gives feedback that there are Maulwurf
        ...
        if player.pos[1] - 1 >= 0 and not (
                map[player.pos[1] - 1][player.pos[0]].cavedIn or map[player.pos[1] - 1][player.pos[0]].hasMaulwurf):
            available_directions.append("n")
        if player.pos[1] + 1 < mapHeight and not (
                map[player.pos[1] + 1][player.pos[0]].cavedIn or map[player.pos[1] + 1][player.pos[0]].hasMaulwurf):
            available_directions.append("s")
        if player.pos[0] + 1 < mapWidth and not (
                map[player.pos[1]][player.pos[0] + 1].cavedIn or map[player.pos[1]][player.pos[0] + 1].hasMaulwurf):
    ...
    elif map[player.pos[1]][player.pos[0]].isExit:  # if the player is on the exit
    ...
    match command:
        ...
        case "quit" | "q":
            if not set(arguments).isdisjoint(["quit", "yes", "y", "q", "game"]):
                ...
                print(
                    "Are you sure you would like to quit? Progress will not be saved...\n(Please type in Quit Game to confirm).")
...
while True:
    ...
    while running:
        ...
        while player.actions_left > 0:
            check_pos(
                player)  # checks if Maulwurf or CaveIn occurred on the player's tile, kicking them to a new tile if so
        ...
        if running:
            ...
            print(ansi.italics(f"\n{ansi.cyan(f"Week {z_calendar[1]}")}") + ansi.italics(
                f": {z_calendar[0]}, {ansi.cyan(f"Day {z_calendar[3]}")} ") +
                  ansi.italics(
                      f"of {z_calendar[4]} ({ansi.cyan(f"{z_calendar[2]}/{z_calendar[3]}/{z_calendar[5]}")}" + ansi.italics(
                          ")")))
