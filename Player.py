...
class Player:
    ...
    def grab_item(self, new_item: Constants.Items, replace_index: int = None):
        if new_item.value != 0: #if you aren't trying to grab nothing
            # First, check if there are any "Nothing" items to auto-replace
            ...
            # If no "Nothing" items and replace_index is specified, replace that specific item
            if replace_index is not None:
                if 0 <= replace_index < len(self.items):
                    ...
                    return dropped_item
            # If no "Nothing" items and no replace_index, return None to indicate user input needed
            return None
